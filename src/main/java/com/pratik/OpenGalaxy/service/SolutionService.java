...
import com.pratik.OpenGalaxy.model.DTOs.SolutionRequestDTO;
import com.pratik.OpenGalaxy.model.DTOs.SolutionResponseDTO;
...
import com.pratik.OpenGalaxy.repository.SolutionRepository;
...
public class SolutionService {
    ...
    // CONSTRUCTOR - Remove CertificateService parameter
    public SolutionService(SolutionRepository solutionRepository,
                           ProblemService problemService,
                           UserService userService,ProblemRepository problemRepository,EmailService emailService) {
    ...
    public Solution createSolution(DefaultOAuth2User oauthUser, String problemId, SolutionRequestDTO dto) {
        try {
            ...
            emailService.sendSolutionSubmittedAlert(savedSolution, problem);
    ...
    public Solution acceptSolution(String solutionId, String githubId) {
        try {
            ...
            if (solution.isAccepted()) {
                ...
                // Increment points for the solution submitter
                ...
                // Check if user earned a new badge and generate certificate if so
                List<String> newBadges = submitter.getBadges();
                if (newBadges.size() > oldBadges.size()) {
                    try {
                        // Find submitter's GitHub ID for certificate generation
                        ...
                        // Don't fail the solution acceptance if certificate generation fails
                    }
                }

                // ✅ Send email alert to solution submitter
                emailService.sendSolutionAcceptedAlert(solution, problem);
            }
